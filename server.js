...
http.listen(3000, function () {
	...
	mongoClient.connect("mongodb://localhost:27017", function (error, client) {
		...
		app.post("/addPost", function (request, result) {
			...
			database.collection("users").findOne({
				...
				if (user == null) {
					...
					//console.log("caption,image,video,type,id:",caption,image,video,type,_id);
					if(type== "page_post"){
						database.collection("pages").findOne({
							...
							if(page==null){
								...
								if(page.user._id.toString()!=user._id.toString()){
									result.json({
										"status":"error",
										"message":"sorry you donot own this page."
									});
									return;
								}
		...
		app.get("/friendsDetails/:id", function (request, result) {
			...
			database.collection("users").findOne({
				...
				// 	result.json({
				// 		"status":"error",
				// 		"message":"User has been loged out. Please login again."
				// 	});
				// } else{
					if(error) return result.status(500).json({message:err.message});

					console.log("my Friend:",frnd);
					result.render("friendsDetails",{frnd:frnd});
				//}
			});
			//result.render("friendsDetails",{data:data});
		});
		...
		app.post("/acceptFriendRequest",function(request,result){
			...
			database.collection("users").findOne({
				...
				if(user==null){
					result.json({
						"status":"error",
						"message":"User has been logedout please login again."
					});
				} else{
					var me=user;
					database.collection("users").findOne({
		...
		app.post("/createPage", function (request, result) {
			...
			database.collection("users").findOne({
				...
				if(user==null){
					result.json({
						"status":"error",
						"message":"User has been loged out. please login first."
					});
				} else{
					if(request.files.coverPhoto.size>0 && request.files.coverPhoto.type.includes("image")){
							...
							if(isLiked){
								database.collection("pages").updateOne({
									...
									database.collection("users").updateOne({
										...
										result.json({
											"status":"Unliked",
											"message":"Pages has been unliked."
										});
									});
								});
							} else{
		...
		app.post("/createGroup",function(request,result){
			...
			database.collection("users").findOne({
				...
				if(user==null){
					result.json({
						"status":"error",
						"message":"User has been loged out. Please login."
					});
				} else{
					if(request.files.coverPhoto.size>0 && request.files.coverPhoto.type.includes("image")){
						coverPhoto="public/images/"+request.files.coverPhoto.name;
			...
			database.collection("groups").findOne({
				...
				if(group==null){
					...
					database.collection("posts").find({
						...
						result.json({
							"status":"success",
							"message":"Recorde has been fetched.",
							"data":group,
							"posts":posts
						});
					});
